...
function usage() {
  return [
    'Armelis Trivy Scan & SIEM Exporter',
    '',
    'Usage:',
    ...
    '  --output <path>          Report path, defaults to .armelis/scans/<timestamp>.<ext>',
...
function outputPath(workspaceRoot, requested, format = 'json') {
  ...
  const relative = requested || path.join('.armelis', 'scans', `${new Date().toISOString().replaceAll(':', '-')}.${ext}`);
